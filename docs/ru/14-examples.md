...
fn eval(link tokens: [Token]) -> int or CalcError {
    ...
    for token in tokens {
        switch token.kind {
            Num => mut stack.push(token.value),
            Add => {
                let b = try stack.pop()
                let a = try stack.pop()
                mut stack.push(a + b)
...
- `fn is_digit(c: u8) -> bool` — проверка, цифра ли символ. Параметр `c` — обычное значение, функция его только читает.
- `fn tokenize(link input: str) -> [Token] or CalcError`:
  - `link input: str` — параметр-ссылка: функция **читает** строку, не потребляет. Вызывающий может передать строку без `databox`.
  - `[Token] or CalcError` — возврат: список токенов **или** ошибка. Механизм ошибок по-Zig.
  - `let tokens: [Token] = {}` — создаём пустой список.
  - `while i < input.len` — цикл по строке. `input.len` — чтение длины (контракт: длина читает).
  - `let c = input[i]` — чтение символа по индексу (контракт: `a[0]` читает, `c` — ссылка на символ).
...
- `switch token.kind` — по виду токена:
  - `Num` — число на стек: `mut stack.push(token.value)`.
  - `Add`/`Sub`/`Mul`/`Div` — берём два числа со стека, считаем, кладём результат:
    - `try stack.pop()` — `pop` возвращает значение **или** ошибку (пустой стек → `StackUnderflow`). `try` пробрасывает ошибку наверх — не нужно писать обработку вручную.
    - `mut stack.push(a + b)` — арифметика потребляет `a` и `b`, `push` потребляет результат.
  - `Div` — дополнительно `if b == 0 { return CalcError.DivByZero }` — деление на ноль ловится явно.
...
**Зона `--- точка входа`** — `fn main()`:
- `let input = "3 4 + 2 *"` — выражение.
- `let tokens = try tokenize(input)` — лексер. `try` пробрасывает ошибку, если она будет.
- `let result = try eval(tokens)` — вычисление. `eval` читает `tokens` через `link` — передача без `databox`.
- `std.print(result)` — вывод.

**Поток данных:** `main` → `tokenize` (строка → токены) → `eval` (токены → число) → `print`. Каждая функция делает одно дело, ошибки пробрасываются через `try`, контракты видны в сигнатурах.
...
fn emit(databox p: Parser, node: Node) -> (int, Parser) {
    let idx = p.nodes.len
...
fn parse_factor(databox p: Parser) -> (int, Parser) or ParseError {
    ...
    switch tok.kind {
        Num => {
            let p1 = advance(p)
            emit(p1, Node { kind: NodeKind.Num, op: Num, value: tok.value, left: -1, right: -1 })
        },
        Sub => {
            let p1 = advance(p)
            let (operand, p2) = try parse_factor(p1)
            emit(p2, Node { kind: NodeKind.Neg, op: Sub, value: 0, left: operand, right: -1 })
        },
        LParen => {
            let p1 = advance(p)
            let (inner, p2) = try parse_expr(p1)
            let closing = peek(p2)
...
fn parse_term(databox p: Parser) -> (int, Parser) or ParseError {
    let (left, p1) = try parse_factor(p)
    term_tail(left, p1)
}

fn term_tail(left: int, databox p: Parser) -> (int, Parser) or ParseError {
    let tok = peek(p)
    if tok.kind != Mul and tok.kind != Div {
        return (left, p)
    }
    let p1 = advance(p)
    let (right, p2) = try parse_factor(p1)
    let (idx, p3) = emit(p2, Node { kind: NodeKind.Bin, op: tok.kind, value: 0, left: left, right: right })
    term_tail(idx, p3)
}

fn parse_expr(databox p: Parser) -> (int, Parser) or ParseError {
    let (left, p1) = try parse_term(p)
    expr_tail(left, p1)
}

fn expr_tail(left: int, databox p: Parser) -> (int, Parser) or ParseError {
    let tok = peek(p)
    if tok.kind != Add and tok.kind != Sub {
        return (left, p)
    }
    let p1 = advance(p)
    let (right, p2) = try parse_term(p1)
    let (idx, p3) = emit(p2, Node { kind: NodeKind.Bin, op: tok.kind, value: 0, left: left, right: right })
    expr_tail(idx, p3)
}
...
fn parse(link input: str) -> (int, [Node]) or ParseError {
    let tokens = try lex(input)
    let p = Parser { tokens: tokens, nodes: {}, pos: 0 }
    let (root, p1) = try parse_expr(p)
    let last = peek(p1)
    if last.kind != Eof {
        return ParseError.UnexpectedToken
    }
...
fn main() {
    let (root, nodes) = try parse("3 + 4 * (2 - 1)")
    std.print(root)
    std.print(nodes.len)
}
```
...
- `fn emit(databox p: Parser, node: Node) -> (int, Parser)` — добавляет узел в арену (`mut p.nodes.push(node)`), возвращает **кортеж**: индекс нового узла + новое состояние парсера.
...
- `parse_factor` — самый низкий уровень: число, унарный минус или выражение в скобках.
  ...
  - `Sub` → унарный минус: разбираем операнд рекурсивно (`try parse_factor(p1)`), создаём узел `Neg`.
  - `LParen` → разбираем выражение внутри скобок (`try parse_expr(p1)`), проверяем закрывающую скобку (`if closing.kind != RParen`), иначе — ошибка.
...
**Зона `--- точка входа`**:
- `fn parse(link input: str) -> (int, [Node]) or ParseError`:
  - `let tokens = try lex(input)` — лексер.
  - `let p = Parser { tokens: tokens, nodes: {}, pos: 0 }` — начальное состояние парсера.
  - `let (root, p1) = try parse_expr(p)` — разбор всего выражения. `root` — индекс корневого узла в арене.
...
- `fn main()`:
  - `let (root, nodes) = try parse("3 + 4 * (2 - 1)")` — разбор.
...
| `databox param: T` | пример 2 | потребляющий параметр: компилятор решает копия/move |
| `[T]` | оба | список (токены, стек, арена узлов) |
| `try expr` | оба | проброс ошибки одной командой |
| `return Error.Variant` | оба | возврат ошибки |
| `switch x { ... }` | оба | switch-выражение, `else` — ветка по умолчанию |
